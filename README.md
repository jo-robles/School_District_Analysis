...
Before a secondary analysis could be completed, it was necessary to remove the ninth-graders math and reading scores from the ```student_data_df``` pandas DataFrame created from the data sources present within the Resources folder. To do so, the following method was utilized after cleaning the data sources:
```
...
```
We once again utilized the ```.loc``` attribute to locate the data that met our criteria and ```len``` to determine the number of rows of students that met the criteria specified above - which happened to be 461 total students. After this step, we simply removed this set of students from a new total student count to determine the passing math, reading and overall rates within the school district. One important item to note here is that when calculating the number of students without Thomas High School in it, we did not change the original count of students but instead created a new variable containing this information. As the district still contains the same number of students (regardless of academic integrity), for the analysis to be correct, we still had to include the total number of students within our summary. We did so by doing the following:
...
```
As we are concerned about the impact of the passing rates of students, we would utilize this updated student count to determine the new rate of students who passed their reading and math sections. From there, we reviewed the resulting pandas DataFrames to answer the following questions:

* How was the district summary affected?
    * Looking at our new district summary, the following is observed:
    ...
    As demonstrated, the overall average math and reading scores changed very little however the percentage of students passing is somewhat impacted. This does make sense as only 461 student scores removed out of 39,170 total would likely have a minor impact on the overall averages and the passing percentages. 
...
* How was the school summary affected?
    * The school summary is where we can immediately see the impact of the removal of the ninth-grade math and reading scores. Here we can see the old school summary (Thomas High School highlighted):
    ...
    We can see, there is a dramatic difference in the passing percentages between the two DataFrames. In terms of an analysis of the data itself, it appears as though many of the students in the ninth-grade dataset for Thomas High School may have not met the criteria of 70% or higher. Utilizing descriptive statistcs of the ninth-grade scores could provide more information however that is outside the scope of this analysis. 
...
* How did replacing the ninth-grade math and reading scores affect the following:
    * Math and reading scores by grade.
    ...
    And when we compare this to the new pandas DataFrame:
    ...
    * Scores by school spending
        * In terms of spending by school size, we can assume that while averages would not be impacted (as they simply wouldn't be in the calculation of the averages (as demonstrated earlier)), the passing percentages would be. To demonstrate this, we created the bins and averages of the pandas DataFrame:  ```per_school_summary_df``` utilizing the following code:
        ...
        As we know that the overall percentages were impacted from Thomas High School, and further, from  previous pandas DataFrame, we know that the school spends $638.00 per student, we can make an assumption that the impact of removing the ninth-graders from Thomas High School which increased the overall passing percentage, we can make the assumption that it will be the spending range - "$630 - 644" that would be the one impacted and that the impact would likely have raised the overall passing percentages. 
        
    * Scores by school size
        * As with the spending categories, we once again had to utilize ```pd.cut``` to categorize our pandas DataFrame based on three bins: Small (<1000 students), Medium (1000 - 2000 students) and Large (2000-5000 students). Like how we knew what type of spending Thomas High School has per student, we also know the number of students who attend Thomas High School - 1635. As a result, after creating our dataframe:
    ...
    * Scores by school type
        * In this case, we know that Thomas HIgh School is classified as a charter. However, as we already have a "School Type" column in our ```per_school_summary_df``` dataframe, we can simply utilize ```.groupby```. We did so by utilizing the following code:
        ...
        ```
        This code allowed us to pull out the average math score based on the type of school that was present. We repeated this step on each of the columns of data we needed and compiled this into a pandas DataFrame:
...
1. The most immediate change to the analysis would be the top 5 schools within the district. When removing the results from the 9th grade, the overall passing percentage rose dramatically. To investigate this further, utilizing descriptive statistics on the 9th grade math and reading scores would certainly be warranted.
